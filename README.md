- como a vida trabalha 
[English](https://github.com/0Darkn/curriculo-/blob/main/English-README.md)

# Currículo Profissional de José Raposo

## Perfil

Profissional com formação e experiência técnica multidisciplinar nas áreas de biologia, eletricidade, eletrónica, mecânica e informática. Forte base em sistemas operativos e programação, com evolução contínua desde tecnologias clássicas (MS-DOS, código máquina, assembler) até linguagens e sistemas modernos (Linux, Python). Experiência prática em ambientes industriais e técnicos, com foco em resolução de problemas, automação e aprendizagem contínua.

---

## Formação e Experiência Técnica

### Estudos e Trabalho

**1978**

* Biologia
* Eletricidade

**1981**

* Serralharia
* Eletricidade
* Eletrónica

**1982**

* Serralharia mecânica
* Eletrónica digital

**1984**

* Eletromecânica
* Microprocessadores
* Mecânica de fluidos
* Hidráulica
* Pneumática

**2000**

- servidores Windows e linux
- apache, mysql + PHP, html,css, JavaScript
- clusters linux

**2007**

- secund life

**2009**

- opensimulator
- osgrid
- LSL script

**2020**

- Python
---

## Sistemas Operacionais

* Windows
* MS-DOS
* Unix, Solaris (Oracle)
* Linux: SUSE, openSUSE


---

## Programação

* BASIC
* Código máquina
* Assembler (Assembly)
* C
* C++
* Python

---

## Experiência Profissional

* **1982 - 1989 | AMERICO AUGUSTO FERNANDES**
* como serralheiro
* **1989 – 1990 | ROSACO SOC METALOMECANICA LD**
* como serralheiro
* **1989 – 1989 | FERNANDO & CRUZ LD**
* como serralheiro
* **1991 – 1992 | ARB AUTO REBOQUES BARREIRO SA**
* como serralheiro
* **1992 – 1992 | BARRMETAL COOP METALORGICA BARREIRO CRL**
* como serralheiro 
* **1993 – 1993 | TUBICEDE CEDENCIA PESSOAL BARREIRO LD**
* como eletromecanico
* **1995 – 1995 | ** ADECCO RECURSOS HUMANOS - EMPRESA DE TRABALHO TEMPORARIO**
* como eletromecanico
* **2000 – 2001 | ** FREGUESIA DE SANTO ANDRE**
* assistente operacional
* **2004 – 2004 | ** SULUS MATERIAIS ELECTRICOS SUL LDA**
* como electricista
* **2005 – 2006 | ** PERSONA - ASSOCIAÇÃO PARA A PROMOÇÃO DA SAUDE**
* como informatico
* **20015 – 2022 | ** MUNICIPIO DO BARREIRO**
* assistente operacional
---

## Competências Técnicas

* Eletricidade e eletrónica analógica e digital
* Eletromecânica industrial
* Sistemas de controlo e automação
* Leitura e interpretação de esquemas elétricos e mecânicos
* Programação de baixo e alto nível
* Sistemas operativos e administração básica
* Engenharia reversa

---

## Competências Pessoais

* Pensamento analítico e lógico
* Capacidade de aprendizagem autónoma
* Resolução de problemas técnicos
* Versatilidade e adaptação a diferentes tecnologias

---

